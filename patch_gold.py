...
FIELD_NAMES = [b"m_gold"]

def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Naheulbeuk gold in GZIP payload")
    parser.add_argument("save_file", help="Path to input save file")
    parser.add_argument("gold_amount", nargs="?", type=int, help="New gold value (uint32)")
    parser.add_argument("--output", help="Output file path (default: <save>.gold.patched)")
    parser.add_argument(
        "--max-hits",
        type=int,
    ...
    parser.add_argument(
        ...
        action="append",
        type=int,
        help="Alias for --slot (for consistency with stats/perks commands)",
    )
    parser.add_argument(
        "--list",
        action="store_true",
        help="List available gold slots with current values and exit",
    )
    args = parser.parse_args()
    print(
        "Deprecated expert helper: use `python3 -m uese naheulbeuk show-stats/patch` for semantic editing; "
        "`patch_gold.py` remains raw economy tooling only."
    )

    save_path = Path(args.save_file)
    output_path = Path(args.output) if args.output else save_path.with_suffix(save_path.suffix + ".gold.patched")
    ...
    if args.gold_amount is None:
        parser.error("gold_amount is required unless --list is used")
    ...
    print(f"Patched {len(hits)} gold field(s).")
